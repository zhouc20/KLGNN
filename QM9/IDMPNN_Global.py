...
class IDMPNN_Global_parallel(nn.Module):
    ...
    def __init__(self,
        ...
        self.setmlp4 = nn.Sequential(nn.Linear(hid_dim_global, hid_dim_global), nn.ReLU(inplace=True),
                          nn.Linear(hid_dim_global, hid_dim), nn.LayerNorm(hid_dim),
                          nn.ReLU(inplace=True))
        ...
        if max_nodez is not None:
            ...
            self.inmlp = self.inmlp = nn.Sequential(nn.Linear(in_dim, hid_dim), nn.ReLU(inplace=True),
                          nn.Linear(hid_dim, hid_dim), nn.LayerNorm(hid_dim),
                          nn.ReLU(inplace=True))
        if max_edgez is not None:
            self.edge_emb = nn.Embedding(max_edgez + 1, hid_dim, padding_idx=0)
    ...
    def forward(self, x: Tensor, adj: SparseTensor, subgs: Tensor,
        ...
        for b in range(B):
            if num_subg[b] == 0:
                continue
            n = int(self.rate * num_subg[b]) + 1 #if self.training else num_subg[b]
...
class IDMPNN_parallel_aggregate(nn.Module):
    num_layer: Final[int]
    ...
    def __init__(self,
        ...
        self.outmlp = nn.Sequential(nn.Linear(hid_dim, out_dim))
        assert cat in ['add', 'hadamard_product']
        self.cat = cat
        assert node_pool in ['max', 'add', 'mean']
        assert subgraph_pool in ['max', 'add', 'mean']
        assert global_pool in ['max', 'add', 'mean', 'attention', 'mix']
    ...
    def forward(self, x: Tensor, subadj: SparseTensor, subgs: Tensor,
        ...
        x1 = self.inmlp(x)  # (B, Nm, hid_dim)
        x_temp = x1[subgbatch]  # (ns, Nm, hid_dim)
        x = torch.zeros([B, Nm, self.hid_dim], requires_grad=True, device=x_temp.device)
        x = x[subgbatch]  # (ns, Nm, hid_dim)
        z = torch.zeros(x.shape, requires_grad=True, device=x.device)
        if self.training:
            z = z[subgbatch].unsqueeze(-2).repeat(
                1, 1, self.drop_permdim, 1)  # (ns1+ns2+...+nsB, Nm, permdim, hid_dim)
        else:
        ...
        select_perm = random.sample(range(self.permdim), self.drop_permdim) if self.training else range(self.permdim)
        perm = self.allperm[:, select_perm]
        z[labelbatch, labelnodes] = self.idemb(perm[nodeidx])  # (selectiondim, drop_perdim, hid_dim)
        x[labelbatch, labelnodes] = x_temp[labelbatch, labelnodes]  # (selectiondim, hid_dim)
        # print(z.shape)
        # print(x.shape)
        ...
        if subadj.dtype == torch.long:
            ...
            subadj = subadj.unsqueeze_(-1)  # subadj (B, Nm, Nm, d/1)
        subadj_ = subadj[subgbatch]  # subadj (ns, Nm, Nm, d/1)
        ...
        for _ in range(self.num_layer):
            x = x + self.graph_mlps[_](torch.einsum("bijd,bjd->bid", subadj_, x))  # (ns, Nm, d)
            if self.drop_ratio > 0:
                x = F.dropout(x, self.drop_ratio, self.training)
            # x[null_node_mask[subgbatch[trn_idx]]] = 0
            x1 = x1 + self.node_pool(x.reshape(x.size(0), -1), subgbatch[trn_idx]).reshape(B, Nm, self.hid_dim)  # (B, Nm, d)
            x1 = x1 + self.graph_mlps_aggregate[_](torch.einsum("bijd,bjd->bid", subadj, x1))  # (B, Nm, d)
            x1 = F.dropout(x1, self.drop_ratio, self.training)
            # x = x + x1[subgbatch[trn_idx]]  # (ns, Nm, d)  # used in bi-aggregate

        for _ in range(self.num_layer_id):
            z = z + self.id_mlps[_](noindexbmm(subadj_, z))  # (ns, Nm, k!, d)
            z = F.dropout(z, self.drop_ratio, self.training)
        z[null_node_mask[subgbatch][trn_idx]] = 0  # should not be here?
        z = self.setmlp1(z.mean(dim=2))  # (ns, Nm, d)
        x = x1[subgbatch[trn_idx]]  # (ns, Nm, d)  use whole graph feature, otherwise use subgraph features
        if self.cat == 'add':
            x = x + z  # 比例？乘法？
        else:
            x = x * z
...
class IDMPNN_parallel_biaggregate(nn.Module):
    num_layer: Final[int]
    ...
    def __init__(self,
        ...
        self.idemb = nn.Embedding(k + 2, hid_dim)
        self.permdim = factorial(k)
        self.drop_permdim = int(self.permdim * perm_drop)
        allperm = extracttuple(torch.arange(k), k)
        self.register_buffer("allperm", allperm.t())  # (k, k!)
        self.num_layer = num_layer
        self.num_layer_global = num_layer_global
    ...
    def forward(self, x: Tensor, subadj: SparseTensor, subgs: Tensor,
        ...
        select_perm = random.sample(range(self.permdim), self.drop_permdim) if self.training else range(self.permdim)
        perm = self.allperm[:, select_perm]
        z[labelbatch, labelnodes] = self.idemb(perm[nodeidx])  # (selectiondim, drop_permdim, hid_dim)
        x[labelbatch, labelnodes] = x_temp[labelbatch, labelnodes]  # (selectiondim, hid_dim)
        # print(z.shape)
        # print(x.shape)
...
class IDMPNN_parallel_AK(nn.Module):
    num_layer: Final[int]
    ...
    def __init__(self,
        ...
        self.graph_mlps_aggregate = nn.ModuleList([
            nn.Sequential(nn.Linear(2*hid_dim, hid_dim), nn.ReLU(inplace=True),
                          nn.Linear(hid_dim, hid_dim), nn.LayerNorm(hid_dim),
                          nn.ReLU(inplace=True)) for _ in range(num_layer)
        ])
        self.id_mlps = nn.ModuleList([
    ...
    def forward(self, x: Tensor, adj: SparseTensor, subgs: Tensor,
        ...
        x1 = self.inmlp(x)  # (B, Nm, hid_dim)
        x1 = x1[subgbatch]  # (ns, Nm, hid_dim)
        z = torch.zeros(x1.shape, requires_grad=True, device=x1.device)
        if self.training:
            z = z[subgbatch].unsqueeze(-2).repeat(
                1, 1, self.drop_permdim, 1)  # (ns1+ns2+...+nsB, Nm, permdim, hid_dim)
        else:
        ...
        ns = 0
        trn_idx = []
        select_ns = torch.zeros([B], device=x.device)
        for b in range(B):
            trn_idx.append(random.sample(range(ns, ns + num_subg[b]), int(self.rate * num_subg[b]) + 1))
            ns += num_subg[b]
            select_ns[b] = int(self.rate * num_subg[b]) + 1
        # trn_idx = random.sample(range(ns), int(self.rate * ns))
        trn_idx = list(_flatten(trn_idx))
        ...
        select_subgidx = torch.arange(len(trn_idx), device=x.device)
        labelbatch = select_subgidx.reshape(-1, 1).repeat(1, self.k).reshape(-1)
        labelnodes = labelnodes.reshape(subgbatch.shape[0], self.k)[trn_idx].reshape(-1)  # (\sum select_ns * k)
        node_label_divide_times = torch.zeros([B, Nm], device=x1.device)
        start_ns = 0
        for b in range(B):
            for ni in range(Nm):
                # a = labelnodes[int(start_ns*self.k): int((start_ns+select_ns[b])*self.k)]
                node_label_divide_times[b, ni] = 1. / (labelnodes[int(start_ns*self.k): int((start_ns+select_ns[b])*self.k)] == ni).sum()
            start_ns += select_ns[b]

        for _ in range(self.num_layer):
            x = torch.zeros([B, Nm, self.hid_dim], requires_grad=True, device=x1.device)
            x = x[subgbatch[trn_idx]]  # (ns, Nm, hid_dim)
            x[labelbatch, labelnodes] = x1[labelbatch, labelnodes]  # (selectiondim, hid_dim)
            x = x + self.graph_mlps[_](torch.einsum("bijd,bjd->bid", adj_, x))  # (ns, Nm, d)
            x = F.dropout(x, self.drop_ratio, self.training)
            x[null_node_mask[subgbatch[trn_idx]]] = 0
            subgidx = torch.arange(subgbatch[trn_idx].size(0), device=x.device)
            subgnode_batch = subgidx.reshape(-1, 1).repeat(1, Nm).reshape(-1)
            subgraph_encoding = self.subgraph_pool(x.reshape(-1, self.hid_dim), subgnode_batch).unsqueeze_(1).repeat(1, Nm, 1)  # (ns, Nm, d)
            x_subgraph = torch.zeros(subgraph_encoding.shape, device=subgraph_encoding.device)
            x_subgraph[labelbatch, labelnodes] = subgraph_encoding[labelbatch, labelnodes]  # (ns, Nm, d)
            x_subgraph = global_add_pool(x_subgraph.reshape(x_subgraph.shape[0], -1), subgbatch[trn_idx])
            x_subgraph = x_subgraph.reshape(x_subgraph.shape[0], Nm, self.hid_dim)  # (B, Nm, d)
            # if self.node_pool_type == 'mean':
            x_subgraph = torch.einsum("bnd,bn->bnd", x_subgraph, node_label_divide_times)  # (B, Nm, d)
            x_context = global_add_pool(x.reshape(x.shape[0], -1), subgbatch[trn_idx])
            x_context = x_context.reshape(x_context.shape[0], Nm, self.hid_dim)  # (B, Nm, d)
            # x1 = torch.cat([x_subgraph, x_context], dim=-1)  # (B, Nm, 2d)
            # x1 = self.graph_mlps_aggregate[_](x1)  # (B, Nm, d)
            # x1 = x_subgraph + x_context
            x1 = x_context
            x1 = F.dropout(x1, self.drop_ratio, self.training)
            x1 = x1[subgbatch[trn_idx]]  # (ns, Nm, d)
        ...
        z[null_node_mask[subgbatch][trn_idx]] = 0  # should not be here?
        z = self.setmlp1(z.mean(dim=2))  # (ns, Nm, d)
        x = x1  # (ns, Nm, d)  use whole graph feature, otherwise use subgraph features
        if self.cat == 'add':
            x = x + z  # 比例？乘法？
        else:
            x = x * z
