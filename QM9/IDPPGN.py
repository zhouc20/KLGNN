...
class IDPPGN(nn.Module):
    def __init__(self, k, nin, nhid, nout, nlayer, depth_of_mlp=2, rate=0.1, cat='add', max_edgez=None):
        ...
        self.inmlp = nn.Sequential(nn.Linear(nin, nhid), nn.ReLU(inplace=True),
                          nn.Linear(nhid, nhid), nn.LayerNorm(nhid),
        ...
        if max_edgez is not None:
            self.edge_emb = nn.Embedding(max_edgez + 1, nhid, padding_idx=0)
    ...
    def forward(self, x, adj: Tensor, subgs: Tensor,
        ...
        adj = adj[subgbatch]
        if self.cat == 'add':
            x[labelbatch, labelnodes] = x[labelbatch, labelnodes] + self.idemb(self.allperm[nodeidx])
        ...
        mask_adj = mask_adj[subgbatch[trn_idx]]
        ...
        x = torch.cat([diag_x, offdiag_x], dim=1)
        x = self.norm(x).transpose(1, 2) # ns * #perm x N_max x 2F

        # to sparse x
        pool_batch = subgbatch[trn_idx].unsqueeze(1).repeat(1, self.permdim).reshape(-1)
        x = global_mean_pool(x.reshape(-1, Nm * 2 * self.nhid), pool_batch)
        x = x.reshape(B * Nm, 2 * self.nhid)  #[mask_x.reshape(-1)] # BN x F
        subgnode_batch = torch.arange(B, device=x.device).unsqueeze(1).repeat(1, Nm).reshape(-1)
        x = global_add_pool(x, subgnode_batch)
        # transform feature by mlp
