...
def graph2IDsubgraph_global_new(data: PygData,
    ...
    return PygData(x=x,
                   ...
                   num_subg=torch.tensor((subgs.shape[0]), dtype=torch.long),
                   num_node=torch.tensor((N), dtype=torch.long),
                   num_edge=torch.tensor((data.edge_index.shape[1]), dtype=torch.long))
...
def graph2IDsubgraph_cluster(data: PygData,
    ...
    #                    col=data.edge_index[1],
    #                    value=data.edge_attr + 1,
    #                    sparse_sizes=(222, 222)).coalesce()
    # adj = adj.to_dense().unsqueeze_(0).to(torch.long)
    # x = torch.cat((data.x, torch.zeros((222 - N, d),
    #                                    dtype=data.x.dtype))).unsqueeze_(0)

    # convert to networkx, current version without node and edge features
    ...
    # edge_list = []
    for j in range(data.edge_index.shape[1]):
        G.add_edge(data.edge_index[0][j].item(), data.edge_index[1][j].item(), bond=data.edge_attr[j, 0] + 1.)
