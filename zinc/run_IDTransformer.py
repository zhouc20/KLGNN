...
parser.add_argument('--use_RWSE', action='store_true', default=False)  # unnecessary
...
t1 = time.time()
processed_path = f"data/{args.dataset}_{k}_{args.distlimitl}_{args.distlimitu}_res_{args.resolution}_rw_{args.rw_steps}.pt" if not args.no_cluster else f"data/{args.dataset}_{k}_{args.distlimitl}_{args.distlimitu}_rw_{args.rw_steps}.pt"
...
record_path = 'IDGPS' + ('_full_graph' if args.full_graph else '') + ('_no_cluster' if args.no_cluster else '') + args.name + '_' + str(args.dataset) \
              + (('/GPS_' + str(args.num_head)) if args.transformer else '') + '_res_' + str(args.resolution) + '_k_' + str(args.k) + '_u_' + str(args.distlimitu) + '_l_' + str(args.distlimitl) + ('_discrete_' if args.discrete else '') + '_pool_node_' + str(args.node_pool) + '_subg_' + str(args.subgraph_pool) + '_global_' + str(args.global_pool) + '_rate_' + str(args.rate) + '_perm_' + str(args.drop_perm) + '_hid_dim_' + str(args.hid_dim) \
              + '_rwstep_' + str(args.rw_steps) + '_dim_' + str(args.se_dim) + '_type_' + args.se_type + '_layers_' + str(args.num_layers) + '_id_' + str(args.num_layers_id) + '_global_' + str(args.num_layers_global) + "_regre_" + str(args.num_layers_regression) + '_cat_' + args.cat + '_final_' + args.final_concat + '_norm_' + args.norm_type + ('_MPNN' if args.local_MPNN else '') + ('_central' if args.central_encoding else '') + ('_bias' if args.attn_bias else '')\
              + '/lr_' + str(args.lr) + ('_cos-lr_' if args.cos_lr else '') + (('_Plateau_patience_' + str(args.patience) + '_factor_' + str(args.factor)) if not args.cos_lr else '') + '_decay_' + str(args.weight_decay) + '_epochs_' + str(args.epochs) + '_bs_' + str(args.batch_size) + ('_ensemble_train' if args.ensemble_train else '') + (('_ensemble_test_' + str(args.sample_times) + '_' + args.ensemble_mode) if args.ensemble_test else '') + '_dropout_' + str(args.drop_ratio) + '_attn_' + str(args.attn_drop) + '_no_' + str(args.no)
...
def buildMod(k, hid_dim, out_dim, num_layer, num_layer_global, num_layer_id, num_layer_regression, node_pool, subgraph_pool, global_pool, bi_aggregate, aggregate, ak, rate, drop_perm, mask_value, cat, drop_ratio, norm, dataset):
    ...
    if dataset.data.adj.dtype == torch.long:
        ...
        print("use edge attr")
    if args.transformer:
        return IDMPNN_Transformer(k, in_dim, hid_dim, out_dim, num_layer, num_layer_global, num_layer_id, num_layer_regression, max_nodez, max_edgez, node_pool, subgraph_pool, global_pool, rate, cat, drop_ratio, args.attn_drop, drop_perm, norm, args.ensemble_test, args.final_concat, args.num_head, args.local_MPNN, args.central_encoding, args.attn_bias, args.rw_steps, args.se_dim, args.se_type)
    else:
...
def train(mod, opt: AdamW, dl):
    ...
    for dat in dl:
        opt.zero_grad()
        x, subgs, adj, y, num_subg, num_node, distance, degree, rwse = dat.x, dat.subgs, dat.adj, dat.y, dat.num_subg, dat.num_node, dat.distance, dat.degree, dat.rwse
        pred = mod(x, degree, rwse, adj, distance, subgs, num_subg, num_node)
